...
const items: any = [
    ...
    { title: 'Nhóm', href: '/news/news-group' },
].map((item, index) => (
    <Box key={index}>
        <Link
...
const NewsGroup = () => {
    ...
    const columns: ColumnDef<TCustomer, any>[] = [
        ...
        columnHelper.accessor((row) => row.title, {
            ...
            header: () => (
                <Text className="text-sm font-semibold">Tên nhóm</Text>
            ),
        }),
    ...
    return (
        <Box className="mx-10 mb-24 px-6 py-8 bg-white shadow-sm rounded-xl max-h-max border border-dashed border-primary-border ">
            ...
            >
                {!isFetching && <ViewGroup newDetail={investorDetail} />}
            </Modal>
