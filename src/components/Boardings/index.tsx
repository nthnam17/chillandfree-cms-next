...
const Boardings = ({ idBuilding }: BoardingsProps) => {
    const items: any = [
        { title: 'Trang chủ', href: '/' },
        { title: 'Chủ đầu tư, chủ trọ', href: '/partners/Investor_list' },
        { title: 'Tòa Nhà', href: `/buildings/${idBuilding}` },
        { title: 'Phòng trọ' },
    ].map((item, index) => (
        <Box key={index}>
            {item.href ? (
    ...
    const [query, setQuery] = useState<TBoardings>({
        status: '',
        name: '',
        page: 1,
        pageSize: 10,
    ...
    const columnHelper = createColumnHelper<TBoardings>();
    ...
    const columns: ColumnDef<TBoardings, any>[] = [
            ...
            >
                <Box>
                    <Title className="text-lg text-primary-black uppercase">
                        Danh Sách phòng trọ của tòa nhà
                    </Title>
                    <Breadcrumbs
                        mt="xs"
                        separator={
