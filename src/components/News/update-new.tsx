...
const UpdateNew = ({ onClose, newDetail }: Props) => {
    const [activeTab, setActiveTab] = useState<string | null>(null);

    const [defaultData, setDefaultData] = useState(newDetail);
    ...
    useEffect(() => {
        if (newDetail) {
            setDefaultData(newDetail);
            reset(newDetail);
            setActiveTab(newDetail?.newDetailList[0]?.languageCode);
        }
    }, [reset, newDetail]);

    useEffect(() => {
        if (newDetail) {
            setValue('image', newDetail.image);
        }
    }, [newDetail]);
